...
This repository contains the finished reference implementation for a 1.5-day hands-on training focused on AI-assisted end-to-end testing with Playwright, MCP tooling, and CI/CD automation. All workshop assets (tests, prompts, slides, exercises) live here so you can demo the end state or use it as inspiration for your own curriculum.

**Workshop Structure:** See `WORKSHOP_STRUCTURE.md` for a clear overview of topics covered in Day 1 vs. Day 2, and how MCP, agents, and CI/CD are divided between the days.
...
tests/
  examples/
    day1/        # Manual baseline + AI-assisted prompts + AI-generated edge cases
    day2/        # Advanced flows, agents, CI integration notes + AI-generated checkout
      seed.spec.ts # Seed test for planner/generator agents
...
| Command | Description |
| --- | --- |
| `npm test` | Run the entire Playwright suite under `tests/examples/` |
...
- `tests/examples/day1/login.spec.ts`: canonical login automation for [https://www.saucedemo.com/](https://www.saucedemo.com/), including baseline assertions and negative paths.
- `tests/examples/day1/login-pom.spec.ts`: example test using Page Object Model pattern for cleaner, more maintainable code.
- `tests/pages/`: Page Object Model classes (`LoginPage`, `InventoryPage`, `CartPage`, `CheckoutPage`) that encapsulate page-specific logic. See `tests/pages/README.md` for usage examples.
- `tests/examples/day1/prompts.md`: prompt engineering walkthrough for generating the login flow with an AI assistant; includes review tips and red-team prompts.
- `scripts/mcp-login-demo.ts`: minimal MCP-style session that opens the site, interacts through tool call equivalents (`goto`, `getByPlaceholder`, `locator.fill`, `expect`), captures a screenshot, trace, video, and structured transcript. **Note:** Video recording is only available via this custom script.
- `artifacts/`: output directory for MCP transcripts (`mcp-transcript.json`), screenshots, videos (from custom script only), and `trace.zip`. The folder is ignored by Git so your local artifacts do not pollute version control.

## Day 2 content

- `tests/examples/day1/login-edge-cases.spec.ts`: example of test cases extended by AI (problem_user, performance_glitch_user, bad credentials). Use it to discuss validation of AI output.
- `tests/examples/day2/checkout-happy-path.spec.ts`: example of checkout flow test generated by Playwright agents.
- `tests/examples/day2/enhancing.md`: facilitator notes for the AI-powered edge-case session (promotion criteria, transcript audit).
...
3. Share `artifacts/mcp-transcript.json`, `artifacts/screenshots/`, `artifacts/videos/`, and `artifacts/trace.zip` with attendees.

This approach gives you full control over the workflow and is useful for understanding the underlying Playwright APIs. **Note:** Video recording is only available via this custom script, not through the official MCP server.
...
- Output directory for artifacts (e.g., `artifacts-mcp-custom-config`)
...
For live coding, create a clean branch (e.g. `walkthrough/day1`) and progressively reintroduce folders/scripts in front of the audience:

1. Start from bare npm + `.gitignore`.
2. Add Playwright config, write the manual login test.
3. Introduce AI prompts and MCP demo.
4. Switch to Day 2 branch for agents, AI-generated specs, and CI.

Documenting this process in the README ensures future trainers know how to reset the repo for interactive sessions without losing the finished solution.
